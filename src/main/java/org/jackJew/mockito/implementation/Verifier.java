...
public class Verifier<T> {
  ...
  public class InterfaceVerifyCallBack implements InvocationHandler {
    ...
    public Object invoke(Object proxy, Method method, Object[] args) {
      ...
      if (executionCount == null || executionCount == 0) {
        if (count != 0) {
          throw new IllegalArgumentException("verify fail[interface], expect " + count + ", but actually is 0");
        }
      } else if (count != executionCount) {
        throw new IllegalArgumentException("verify fail[interface], expect " + count + ", but actually is " + executionCount);
      }
      return null;
    }
  }
