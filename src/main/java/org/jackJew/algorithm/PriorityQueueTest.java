...
public class PriorityQueueTest {
	...
	public static void main(String[] args) {
		...
		pq.add(4);
		pq.add(-5);
		// add: siftUp,  k = size,  (k-1) >> 1; while loop, find where greater than/equal to its parent, or is the root
		// add: always maintain the 0-index element as the least by siftUp method.
		
		Integer ele = 0;
		while( (ele = pq.poll()) != null ) {
			// poll: siftDown,  k = 0 ; (k+1) << 1,while loop, find where less than/equal to its children, or is the leaf
